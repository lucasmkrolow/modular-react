...
function Post() {
  ...
  console.log(token);
  const url =
    "https://modularathis.sharepoint.com/sites/Website/_api/web/lists/getbytitle('Páginas do Site')/GetItemById(" +
    idDoPostParam +
    ")";
  const accessToken =
    ...
    <div>
      ...
      {Object.values(posts).map((post) => (
        <div style={{ padding: "10px" }}>
          <div dangerouslySetInnerHTML={{ __html: post.CanvasContent1 }}></div>
        </div>
      ))}
    </div>
  );
